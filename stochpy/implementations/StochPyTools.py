...
class StochPySSA_Shared():
    def Parse(self, model_file, model_dir, IsTauleaping=False, IsNRM=False, IsDelayed=False, IsSMM=False,
              IsQuiet=False):
        """
        ...
        """
        try:
            self.parse = PySCeS_Connector(model_file, model_dir, IsTauleaping=IsTauleaping, IsNRM=IsNRM,
                                          IsDelayed=IsDelayed, IsSMM=IsSMM, IsQuiet=IsQuiet)  # Parse model
            ...
            self.rate_pos = {r_id: j for j, r_id in
                             enumerate(self.rate_names)}  # Determine once for each rate it's position
            ...
            self.species_pos = {s_id: i for i, s_id in enumerate(
                self.species_names)}  # Determine once for each species (variable, assigned, fixed) it's position
            ...
            print(er)
            print(
                "Error: StochPy failed parsing input file '{0:s}' from directory '{1:s}'".format(model_file, model_dir))
            sys.exit()
    ...
    def SetEvents(self):
        ...
        for ev in self.__events__:
            for s_id in sorted(self.species_names,
                               reverse=True):  # makes sure that the longest identifiers are replaced first
                if s_id not in self.fixed_species:
                    ev.code_string = ev.code_string.replace('self.mod.{0:s}'.format(s_id),
                                                            'X_matrix[{0:d}]'.format(self.species_pos[s_id]))
    ...
    def Propensities(self, IsTauleaping=False):
        ...
        if self._IsInitial:
            ...
            [setattr(__species__, self.parse.species[s], self.X_matrix[s]) for s in
             range(self.n_species)]  # Set species quantities
            [setattr(__species__, self.fixed_species[s], self.fixed_species_amount[s]) for s in
             range(len(self.fixed_species))]
    ...
    def BuildPropensityCodes(self, propensities=None):  # 21-11-2013
        ...
        code_str_all += '\n'.join(
            ['r_vec[{0:d}]={1:s}'.format(i, propensity) for i, propensity in enumerate(propensities)])
            ...
            if IsTrigger:
                if '_TIME_' in ev.symbols and len(ev.symbols) == 1:  # pure time event
                    ...
                    if not ev(10 ** -99, self.X_matrix):  # _TIME_ > 3.0
                        ...
                        if np.isnan(
                                self.reaction_index):  # reaction_index = nan, ignore nothing happend (probably the end time is reached)
                            pass
                ...
                if IsModify:
                    for s_id in list(self.__eDict__[ev.name]['assignments']):
                        if s_id not in self.fixed_species:
                            ...
                            try:
                                self.X_matrix[s_index] = float(
                                    int(self.__eDict__[ev.name]['assignments'][s_id]))  # convert to int
                            except ValueError:
                                raise ValueError("Invalid assignment '{0:s}' for identifier {1:s}".format(
                                    self.__eDict__[ev.name]['assignments'][s_id], s_id))
                        else:
                            s_index = self.fixed_species.index(s_id)
                            self.fixed_species_amount[s_index] = float(self.__eDict__[ev.name]['assignments'][
                                                                           s_id])  # march 11, 2015: do not convert to integer, because it could be a parameter which does not have to be an integer
        ...
        if self.sim_t == 0:
            ...
            for s_id in self.parse.species:
                for assign_species in list(self.__aDict__):
                    if s_id in self.__aDict__[assign_species][
                        'formula']:  # if 'normal' species in assignment relationship
    ...
    def rateFunc(self, rate_eval_code, r_vec):
        ...
        try:
            ...
            print(er)
            print(
                "Error: Propensities cannot be determined. Please check if all variable species amounts are initialized")
            sys.exit()
